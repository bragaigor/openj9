...
void
MM_ParallelPartialMarkTask::run(MM_EnvironmentBase *envBase)
{
	MM_EnvironmentVLHGC *env = MM_EnvironmentVLHGC::getEnvironment(envBase);

	env->_workStack.prepareForWork(env, _cycleState->_workPackets);
...
void
MM_ParallelPartialMarkTask::cleanup(MM_EnvironmentBase *envBase)
{
	MM_EnvironmentVLHGC *env = MM_EnvironmentVLHGC::getEnvironment(envBase);
	...
	Trc_MM_ParallelPartialMarkTask_parallelStats(
...
void
MM_ParallelPartialMarkTask::synchronizeGCThreads(MM_EnvironmentBase *envBase, const char *id)
{
	MM_EnvironmentVLHGC *env = MM_EnvironmentVLHGC::getEnvironment(envBase);
	PORT_ACCESS_FROM_ENVIRONMENT(env);
...
bool
MM_ParallelPartialMarkTask::synchronizeGCThreadsAndReleaseSingleThread(MM_EnvironmentBase *envBase, const char *id)
{
	MM_EnvironmentVLHGC *env = MM_EnvironmentVLHGC::getEnvironment(envBase);
	PORT_ACCESS_FROM_ENVIRONMENT(env);
	U_64 startTime = j9time_hires_clock();
 ...
 */
MM_PartialMarkingScheme *
MM_PartialMarkingScheme::newInstance(MM_EnvironmentVLHGC *env)
{
	MM_PartialMarkingScheme *markingScheme;

	markingScheme = (MM_PartialMarkingScheme *)env->getForge()->allocate(sizeof(MM_PartialMarkingScheme), MM_AllocationCategory::FIXED, J9_GET_CALLSITE());
	if (markingScheme) {
		new(markingScheme) MM_PartialMarkingScheme(env);
 ...
 */
void
MM_PartialMarkingScheme::kill(MM_EnvironmentVLHGC *env)
{
	tearDown(env);
 ...
 */
bool
MM_PartialMarkingScheme::initialize(MM_EnvironmentVLHGC *env)
{
 ...
 */
void
MM_PartialMarkingScheme::tearDown(MM_EnvironmentVLHGC *env)
{
}
 ...
 */
bool
MM_PartialMarkingScheme::heapAddRange(MM_EnvironmentVLHGC *env, MM_MemorySubSpace *subspace, UDATA size, void *lowAddress, void *highAddress)
{
 ...
 */
bool
MM_PartialMarkingScheme::heapRemoveRange(MM_EnvironmentVLHGC *env, MM_MemorySubSpace *subspace, UDATA size, void *lowAddress, void *highAddress, void *lowValidAddress, void *highValidAddress)
{
	/* Record the range in which valid objects appear */
	_heapBase = _extensions->heap->getHeapBase();
	_heapTop = _extensions->heap->getHeapTop();
...
MM_PartialMarkingScheme::masterSetupForGC(MM_EnvironmentVLHGC *env)
{
	...
	static_cast<MM_CycleStateVLHGC*>(env->_cycleState)->_vlhgcIncrementStats._markStats._ownableSynchronizerCandidates = ownableSynchronizerCandidates;
}
...
void
MM_PartialMarkingScheme::workerSetupForGC(MM_EnvironmentVLHGC *env)
{
...
void
MM_PartialMarkingScheme::setCachedState(MM_MarkMap *markMap, bool dynamicClassUnloadingEnabled)
{
...
void
MM_PartialMarkingScheme::clearCachedState()
{
	_markMap = NULL;
}
 ...
 */
bool
MM_PartialMarkingScheme::isMarked(J9Object *objectPtr)
{
...
MMINLINE bool
MM_PartialMarkingScheme::markObjectNoCheck(MM_EnvironmentVLHGC *env, J9Object *objectPtr, bool leafType)
{
...
MMINLINE bool
MM_PartialMarkingScheme::markObject(MM_EnvironmentVLHGC *env, J9Object *objectPtr, bool leafType)
{
	bool didMark = false;
...
MMINLINE void
MM_PartialMarkingScheme::rememberReferenceIfRequired(MM_EnvironmentVLHGC *env, J9Object *from, J9Object *to)
{
	if (NULL != to) {
...
MMINLINE void
MM_PartialMarkingScheme::markObjectClass(MM_EnvironmentVLHGC *env, J9Object *objectPtr)
{
#if defined(J9VM_GC_DYNAMIC_CLASS_UNLOADING)
...
MMINLINE void
MM_PartialMarkingScheme::updateScanStats(MM_EnvironmentVLHGC *env, UDATA bytesScanned, ScanReason reason)
{
...
MMINLINE void
MM_PartialMarkingScheme::addOwnableSynchronizerObjectInList(MM_EnvironmentVLHGC *env, j9object_t object)
{
	if (NULL != _extensions->accessBarrier->isObjectInOwnableSynchronizerList(object)) {
		...
		env->_markVLHGCStats._ownableSynchronizerSurvived += 1;
	}
}
/****************************************
 * Scanning
 ...
 */
void
MM_PartialMarkingScheme::scanMixedObject(MM_EnvironmentVLHGC *env, J9Object *objectPtr, ScanReason reason)
{
...
void
MM_PartialMarkingScheme::scanReferenceMixedObject(MM_EnvironmentVLHGC *env, J9Object *objectPtr, ScanReason reason)
{
	fj9object_t *endScanPtr;
	UDATA *descriptionPtr;
	UDATA descriptionBits;
	...
	/* we can tell if the reference object is in the collection set if it came from a packet or overflowed region. Dirty cards (in PGC) are only used for remembered objects. */
	bool isReferenceInCollectionSet = (SCAN_REASON_DIRTY_CARD != reason);
...
UDATA
MM_PartialMarkingScheme::scanPointerArrayObjectSplit(MM_EnvironmentVLHGC *env, J9IndexableObject *objectPtr, UDATA startIndex, ScanReason reason)
{
	UDATA slotsToScan = 0;
...
void
MM_PartialMarkingScheme::scanPointerArrayObject(MM_EnvironmentVLHGC *env, J9IndexableObject *objectPtr, ScanReason reason)
{
...
void 
MM_PartialMarkingScheme::scanClassObject(MM_EnvironmentVLHGC *env, J9Object *classObject, ScanReason reason)
{
...
void 
MM_PartialMarkingScheme::scanClassLoaderObject(MM_EnvironmentVLHGC *env, J9Object *classLoaderObject, ScanReason reason)
{
...
void
MM_PartialMarkingScheme::scanClassLoaderSlots(MM_EnvironmentVLHGC *env, J9ClassLoader *classLoader, ScanReason reason)
{
	if(NULL != classLoader){
...
void
MM_PartialMarkingScheme::scanObject(MM_EnvironmentVLHGC *env, J9Object *objectPtr, ScanReason reason)
{
	Assert_MM_true(PACKET_INVALID_OBJECT != (UDATA)objectPtr); /* Invalid objects can only exist on the GMP's work packets */
	...
	switch(_extensions->objectModel.getScanType(clazz)) {
		...
		case GC_ObjectModel::SCAN_PRIMITIVE_ARRAY_OBJECT:
			...
			break;
		default:
			Trc_MM_PartialMarkingScheme_scanObject_invalid(env->getLanguageVMThread(), objectPtr, reason);
			Assert_MM_unreachable();
	}
}
 ...
 */
MMINLINE void
MM_PartialMarkingScheme::completeScan(MM_EnvironmentVLHGC *env)
{
	PORT_ACCESS_FROM_ENVIRONMENT(env);
	do {
...
#if defined(J9VM_GC_FINALIZATION)
void
MM_PartialMarkingScheme::scanUnfinalizedObjects(MM_EnvironmentVLHGC *env)
{
...
/**
 * The root set scanner for MM_MarkingScheme.
 * @copydoc MM_RootScanner
 * @ingroup GC_Modron_Standard
 */
class MM_PartialMarkingSchemeRootMarker : public MM_RootScanner
{
private:
	MM_PartialMarkingScheme *_markingScheme; /**< the MarkingScheme which created this scanner */
...
public:
	...
	MM_PartialMarkingSchemeRootMarker(MM_EnvironmentBase *env, MM_PartialMarkingScheme *markingScheme) :
		  MM_RootScanner(env)
...
/**
 * The clearable reference set scanner for MM_PartialMarkingScheme.
 * @copydoc MM_RootScanner
 * @ingroup GC_Modron_Standard
 */
class MM_PartialMarkingSchemeRootClearer : public MM_RootScanner
{
private:
	...
	virtual void scanOwnableSynchronizerObjects(MM_EnvironmentBase *env) {
		/* allow the marking scheme to handle this, since it knows which regions are interesting */
		/* empty, move ownable synchronizer processing in marking phase */
	}
...
public:
	MM_PartialMarkingSchemeRootClearer(MM_EnvironmentBase *env, MM_PartialMarkingScheme *markingScheme) :
		MM_RootScanner(env),
		_markingScheme(markingScheme)
	{
		_typeId = __FUNCTION__;
 ...
 */
void
MM_PartialMarkingScheme::markLiveObjectsInit(MM_EnvironmentVLHGC *env)
{
	 ...
	 */
	_interRegionRememberedSet->clearFromRegionReferencesForMark(env);

	/* remember reference objects and clear any regions in the previous mark map which are about the be re-marked */ 
	initializeForPartialCollect(env);
	
	env->_currentTask->synchronizeGCThreads(env, UNIQUE_ID);
}

void
MM_PartialMarkingScheme::initializeForPartialCollect(MM_EnvironmentVLHGC *env)
{
	Assert_MM_true(MM_CycleState::CT_PARTIAL_GARBAGE_COLLECTION == env->_cycleState->_collectionType);
	...
	while(NULL != (region = regionIterator.nextRegion())) {
		if (region->_markData._shouldMark) {
			if(J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
				...
				/* remember reference objects from the GMP cycle */
				...
				rememberReferenceList(env, referenceObjectList->getPriorWeakList());
				rememberReferenceList(env, referenceObjectList->getPriorSoftList());
				rememberReferenceList(env, referenceObjectList->getPriorPhantomList());
			}
		}
	}
}
...
void
MM_PartialMarkingScheme::cleanCardTableForPartialCollect(MM_EnvironmentVLHGC *env, MM_CardCleaner *cardCleaner)
{
	PORT_ACCESS_FROM_ENVIRONMENT(env);
...
MM_PartialMarkingScheme::markLiveObjectsRoots(MM_EnvironmentVLHGC *env)
{
	...
	if(isDynamicClassUnloadingEnabled()) {
		...
		if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
					...
					while(NULL != (segment = segmentIterator.nextSegment())) {
						...
						while(NULL != (clazz = classHeapIterator.nextClass())) {
							if (classLoaderRememberedSet->isClassRemembered(env, clazz)) {
								markObject(env, clazz->classObject);
							}
						}
					}
				} else {
			...
			/* also mark the permanent loader just in case they weren't found in the above loop */
...
void
MM_PartialMarkingScheme::markLiveObjectsScan(MM_EnvironmentVLHGC *env)
{
	completeScan(env);
}
 ...
 */
void
MM_PartialMarkingScheme::markLiveObjectsComplete(MM_EnvironmentVLHGC *env)
{
	...
	MM_PartialMarkingSchemeRootClearer rootClearer(env, this);
	rootClearer.setStringTableAsRoot(!isCollectStringConstantsEnabled());
	rootClearer.scanClearable(env);

	/* An in-progress GMP may have discovered objects in the collection set and marked them already. Some
	 * of these objects may have become garbage since the GMP began, and won't have been discovered by the
	 * PGC. Remove these objects from the next mark map.
	 */
	if (NULL != env->_cycleState->_externalCycleState) {
		/* TODO: is this sync necessary, or does completeScan() already guarantee this? */
		/* we need to ensure that we've finished building the mark map before we clean the next mark map */ 
		env->_currentTask->synchronizeGCThreads(env, UNIQUE_ID);
		deleteDeadObjectsFromExternalCycle(env);
	}
}

void 
MM_PartialMarkingScheme::deleteDeadObjectsFromExternalCycle(MM_EnvironmentVLHGC *env)
{
	Trc_MM_PartialMarkingScheme_deleteDeadObjectsFromExternalCycle_Entry(env->getLanguageVMThread());
	...
	while(NULL != (region = regionIterator.nextRegion())) {
		if(region->_markData._shouldMark) {
			if(J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
				UDATA lowIndex = _markMap->getSlotIndex((J9Object*)region->getLowAddress());
				UDATA highIndex = _markMap->getSlotIndex((J9Object*)region->getHighAddress());
				for (UDATA index = lowIndex; index < highIndex; index++) {
					UDATA previousSlot = _markMap->getSlot(index);
					UDATA nextSlot = externalMarkMap->getSlot(index);
					UDATA newNextSlot = nextSlot & previousSlot;
					if (newNextSlot != nextSlot) {
						externalMarkMap->setSlot(index, newNextSlot);
					}
				}
			}
		}
			...
			if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
				...
				while (NULL != (slot = slotIterator.nextSlot())) {
					...
					if (PACKET_INVALID_OBJECT != (UDATA)object) {
						totalCount += 1;
						/* The externalMarkMap might not have completed rebuilding yet, since there's no sync point
						 * between this loop and the one above. Use the previous mark map to check if objects in the
						 * work packet are still live.
						 */ 
	...
	Trc_MM_PartialMarkingScheme_deleteDeadObjectsFromExternalCycle_Exit(env->getLanguageVMThread(), deletedCount, totalCount);
}

bool
MM_PartialMarkingScheme::handleOverflow(MM_EnvironmentVLHGC *env)
{
...
void
MM_PartialMarkingScheme::cleanRegion(MM_EnvironmentVLHGC *env, MM_HeapRegionDescriptorVLHGC *region, U_8 flagToClean)
{
...
void
MM_PartialMarkingScheme::scanObjectsInRange(MM_EnvironmentVLHGC *env, void *lowAddress, void *highAddress, bool rememberedObjectsOnly)
{
...
void
MM_PartialMarkingScheme::scanWeakReferenceObjects(MM_EnvironmentVLHGC *env)
{
...
void
MM_PartialMarkingScheme::scanSoftReferenceObjects(MM_EnvironmentVLHGC *env)
{
	Assert_MM_true(env->getGCEnvironment()->_referenceObjectBuffer->isEmpty());
...
void
MM_PartialMarkingScheme::scanPhantomReferenceObjects(MM_EnvironmentVLHGC *env)
{
...
void
MM_PartialMarkingScheme::processReferenceList(MM_EnvironmentVLHGC *env, J9Object* headOfList, MM_ReferenceStats *referenceStats)
{
	...
	while (NULL != referenceObj) {
		...
		if (GC_ObjectModel::REF_STATE_REMEMBERED == J9GC_J9VMJAVALANGREFERENCE_STATE(env, referenceObj)) {
			Assert_MM_true(NULL != env->_cycleState->_externalCycleState);
...
void
MM_PartialMarkingScheme::rememberReferenceList(MM_EnvironmentVLHGC *env, J9Object* headOfList)
{
	...
	while (NULL != referenceObj) {
		...
		referenceObj = _extensions->accessBarrier->getReferenceLink(referenceObj);
	}
}

void 
MM_PartialMarkingScheme::flushBuffers(MM_EnvironmentVLHGC *env)
{
